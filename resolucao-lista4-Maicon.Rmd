...
glimpse(dados_sm_tbl_lg)

# os dados ainda não estão arrumados, é preciso separar a coluna element
dados_sm_tbl_w <- pivot_wider(
  data = dados_sm_tbl_lg,
  names_from = element,
  values_from = value
)
# verificando o data frame arrumado
glimpse(dados_sm_tbl_w)
```
...
# arrumando os dados cronologicamente
dados_sm_tbl_w %>% select(id, year, everything())

# removendo o character "d" da coluna "day" e convertendo para inteiro
dados_sm_tbl_w$day <- str_replace(dados_sm_tbl_w$day, pattern = "[d]", replacement = "") 

dados_sm_tbl_w$day <- as.integer(dados_sm_tbl_w$day)
...
dados_sm_m <- mutate(dados_sm_tbl_w, tmed = (tmax + tmin)/2)
...
(datas_merj_left)
# Os dados já estão em ordem cronológica
```